        ...
        if(inventory && inventory.container){
            ...
            for(let j = 0; j < container.size; j++) for(let k = 0; k < ids.length; k++){
                ...
                if(item && item.typeId.includes(ids[k])){
                    container.setItem(j);
                    console.error(`Illegal item ${item.typeId} x${item.amount} found in player ${players[i].name} at (${players[i].location.x.toFixed(1)},${players[i].location.y.toFixed(1)},${players[i].location.z.toFixed(1)})`);
